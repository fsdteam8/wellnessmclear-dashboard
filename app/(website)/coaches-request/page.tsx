...
export default function CoachRequestsPage() {
  ...
  const [approvedCoachIds, setApprovedCoachIds] = useState<string[]>([]); // Track approved coaches
  ...
  const updateCoachStatusMutation = useMutation({
    ...
    onSuccess: (_, { coachId }) => {
      ...
      setApprovedCoachIds((prev) => [...prev, coachId]); // Mark coach as approved locally
      ...
      render: (_, row) => (
        <div className="flex items-center space-x-2">
          <Button
            ...
            disabled={processingId === row._id || approvedCoachIds.includes(row._id)}
          >
